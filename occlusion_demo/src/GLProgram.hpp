#ifndef _GLPROGRAM_HPP__
#define _GLPROGRAM_HPP__
...
public:
	...
	Program(std::string vs, std::string gs, std::string fs, std::string feedbackVar) {
		name = glCreateProgram();
		std::string vs_src = readFile(vs);
	...
	inline GLint getUniform(std::string var) {
		...
		if(loc < 0){
            std::cout << "Cannot bind uniform: " << var << std::endl;
        }
		return loc;
	}
	...
	inline void setAttributePointer(std::string var, unsigned int size, GLenum type=GL_FLOAT, GLboolean normalized=GL_FALSE, GLsizei stride=0, GLvoid* pointer=NULL) {
		...
		if(loc < 0){
            std::cout << "Cannot bind attribute: " << var << std::endl;
        }
